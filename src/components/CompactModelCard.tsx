...
export const CompactModelCard: React.FC<CompactModelCardProps> = memo(({
  ...
  return (
    <div
      ...
      className="group bg-zinc-900/80 hover:bg-zinc-900 border border-zinc-800/80 rounded-xl overflow-hidden hover:border-violet-500/50 hover:shadow-xl hover:shadow-violet-950/20 transition-all duration-200 cursor-pointer flex flex-col relative transform-gpu"
    >
      {/* Media Snapshot */}
      <div className="relative aspect-[4/3] bg-zinc-950 overflow-hidden">
        ...
        <div className="absolute top-2 left-2 flex items-center gap-1 z-10">
          <span className="bg-violet-600/90 text-white font-extrabold text-[9px] uppercase px-1.5 py-0.5 rounded flex items-center gap-1 shadow">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/30 backdrop-blur-[1px] z-10">
          <div className="w-9 h-9 rounded-full bg-violet-600 text-white flex items-center justify-center shadow-lg transform scale-90 group-hover:scale-100 transition-transform">
        ...
        <div className="absolute bottom-2 left-2 right-2 flex items-center gap-2 z-10">
          <div className="w-7 h-7 rounded-full border border-violet-500/80 overflow-hidden shrink-0 shadow">
            <img
              src={model.avatarUrl}
              alt={model.displayName}
              loading="lazy"
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[10px] text-zinc-400 truncate flex items-center gap-1">
              ...
              <span>•</span>
              <span className="text-violet-400 font-bold">{model.tokensPerMin} TK/m</span>
            </div>
          </div>
        </div>
      </div>
